...
CREATE TABLE IF NOT EXISTS t_user (
    ...
    waist_circumference DECIMAL(5,2) COMMENT '腰围(厘米)',
    systolic_pressure INT COMMENT '收缩压(mmHg)',
    diastolic_pressure INT COMMENT '舒张压(mmHg)',
    resting_heart_rate INT COMMENT '静息心率(次/分)',
    avg_exercise_heart_rate INT COMMENT '运动平均心率(次/分)',
    max_heart_rate INT COMMENT '运动最高心率(次/分)',
