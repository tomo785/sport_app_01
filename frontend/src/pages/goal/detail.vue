<template>
  <view class="container">
    ...
    <view class="header" :style="{ background: getCoverBg(detail.level) }">
      <AppIcon class="h-icon" :name="getLevelIconName(detail.level)" size="72" />
      ...
      <view class="h-stats">
        <view class="hs-item"><AppIcon name="star-fill" size="22" /><text>{{ (detail.rating || 0).toFixed(1) }}</text></view>
        <view class="hs-item"><AppIcon name="users" size="22" /><text>{{ detail.enrollCount || 0 }}人使用</text></view>
        <view class="hs-item"><AppIcon name="views" size="22" /><text>{{ detail.viewCount || 0 }}次浏览</text></view>
      </view>
      <view class="h-actions" v-if="detail.userStatus !== 1">
        <view class="ha-btn" @click="doAdopt">采用此计划</view>
      </view>
      <view class="h-adopted" v-else>
        <view class="adopted-text"><AppIcon name="check" size="24" /><text>已采用 · 第{{ detail.currentWeek || 1 }}/{{ detail.totalWeeks }}周</text></view>
      </view>
    </view>
    <!-- 课程详情 -->
      ...
      <block v-for="(courses, weekLabel) in groupedCourses" :key="weekLabel">
        <view class="week-section">
          <view class="ws-header" @click="toggleWeek(weekLabel)">
            ...
            <AppIcon class="ws-toggle" :name="expandedWeeks[weekLabel] ? 'arrowUp' : 'arrowDown'" size="24" bold />
          </view>
          <view class="ws-body" v-if="expandedWeeks[weekLabel]">
            <view class="day-group" v-for="(dayCourses, dayLabel) in courses" :key="dayLabel">
              ...
              <view class="course-card" v-for="course in dayCourses" :key="course.id">
                <view class="cc-head">
                  <AppIcon class="cc-icon" :name="getTypeIconName(course.type)" size="32" />
...
function getTypeIconName(type) {
  const icons = { 1: 'run', 2: 'strength', 3: 'stretch', 4: 'hiit', 5: 'shuffle', 6: 'rest', 7: 'shuffle' }
...
function getLevelIconName(level) {
  const icons = { 1: 'beginner', 2: 'intensity', 3: 'strength', 4: 'achievement' }
  return icons[level] || 'beginner'
}
