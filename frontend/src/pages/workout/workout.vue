  ...
  <view class="container">
    ...
    >
      <view class="record-list" v-if="workoutStore.hasRecords">
        ...
        >
          <view class="record-header">
            ...
            <text class="record-date">{{ formatDateTime(item.startTime || item.startTimeStr) }}</text>
          </view>
      ...
      <view class="empty-state" v-else-if="!workoutStore.loading">
        <AppIcon class="empty-icon" name="plan" size="72" />
...
const aiStatsData = computed(() => {
  ...
  return {
    ...
    todayStats: {},
    trendData: {
      dates: records.slice(0, 7).map(r => (r.startTime || r.startTimeStr || '').substring(5, 10)),
...
const onUsePlan = (planData) => {
  if (planData?.action === 'savedWeekly' || planData?.action === 'savedTraining') {
    return
  }
