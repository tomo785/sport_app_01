  ...
  <view class="home">
    ...
    <view class="greeting-row">
      ...
      <view class="greeting-right">
        <AppIcon class="greeting-icon" :name="greetingIcon" size="44" />
      </view>
    </view>
    <!-- ===== 天气卡片 ===== -->
    <view class="weather-card">
      ...
      <view class="weather-right">
        <view class="weather-loc-aqi-row">
          <view class="weather-loc"><AppIcon name="location" size="24" /><text>{{ weather.city || '获取中' }}</text></view>
          <view class="weather-aqi" v-if="weather.aqi">
            <view class="aqi-dot" :style="{ background: weather.aqiColor || 'var(--accent-green)' }"></view>
      ...
      <view class="ac-main">
        ...
        <view class="steps-ring-wrap">
          <svg class="steps-svg" viewBox="0 0 120 120">
            ...
            <circle cx="60" cy="60" r="52" fill="none" stroke="var(--accent-green)" stroke-width="8"
          ...
          <view class="steps-icon-box">
            <AppIcon class="steps-icon" name="steps" size="36" />
          </view>
        </view>
        ...
        <view class="checkin-btn" :class="{ 'checkin-done': checkedIn }" @click="handleCheckin">
          <AppIcon class="checkin-icon" name="check" size="28" />
    ...
    <view class="action-bar">
      <view class="action-btn action-primary" @click="goRunning">
        <AppIcon class="action-icon" name="play-right-fill" size="24" />
    ...
    <view class="session-card">
      <view class="sc-head">
        ...
        <view class="sc-type" @click="goToTodaySession">
          <text>{{ todayStats.recordCount || 0 }} 次运动</text>
          <AppIcon name="arrowRight" size="24" />
        </view>
      </view>
      <!-- 今日计划与小项目 -->
      <template v-if="todayPlan && todayPlan.hasPlan">
        <view class="sc-empty-head">
          <view class="sc-empty-copy">
            <text class="sc-empty-title">{{ todayPlan.name }}</text>
            <text class="sc-empty-sub">{{ sessionCueMeta }}</text>
          </view>
          <view class="sc-empty-link" @click="openTodayPlanExecution(0)">
            <text>执行计划</text>
            <AppIcon name="arrowRight" size="24" />
          </view>
        </view>
        <view class="act-card-list" v-if="todayPlan.activities && todayPlan.activities.length > 0">
          <view
            ...
            @click.stop="openTodayPlanExecution(idx)"
          >
            <view class="act-card-row1">
              <AppIcon class="act-icon" :name="act.icon" size="30" />
      ...
      <view class="sc-empty-head" v-else>
        <view class="sc-empty-copy">
          <text class="sc-empty-title">今天还没有训练计划</text>
          <text class="sc-empty-sub">可以直接开始运动，或去定制一个今日计划</text>
        </view>
        <view class="sc-empty-link" @click="goToCreatePlan">
          <text>去定制</text>
          <AppIcon name="arrowRight" size="24" />
        </view>
      </view>
      ...
      <view class="sc-body">
        <view class="sc-stats-grid">
          ...
          <view class="sc-stat">
            <text class="scs-val">{{ todayCaloriesStr }}</text>
            <text class="scs-label">千卡</text>
          </view>
        </view>
        <view class="sc-action-row">
          <view class="sc-action primary" @click="goRunning">开始运动</view>
          <view class="sc-action" @click="goToSessionPlanAction">{{ todayPlan?.hasPlan ? '执行计划' : '去定制' }}</view>
        </view>
      </view>
    </view>
    <!-- ===== AI 教练卡片 ===== -->
    <view class="ai-coach-card">
      <view class="ai-coach-header">
        <view class="ai-coach-title-row">
          <AppIcon class="ai-coach-icon" name="ai" size="34" />
...
const todayCaloriesStr = computed(() => {
  const value = todayStats.value.calories || 0
  return value > 0 ? String(Math.round(value)) : '0'
})
const sessionCueTitle = computed(() => {
  if (todayPlan.value?.hasPlan) {
    if (todayPlan.value.planType === 'rest') return '今天是恢复日'
    return todayPlan.value.name || '今日计划已就绪'
  }
  return '今天还没有运动记录'
})
const sessionCueMeta = computed(() => {
  if (todayPlan.value?.hasPlan) {
    if (todayPlan.value.planType === 'rest') return '做一点轻松活动，给身体留出恢复空间'
    const duration = todayPlan.value.duration ? `${todayPlan.value.duration}分钟` : ''
    const count = todayPlan.value.activities?.length ? `${todayPlan.value.activities.length}项训练` : ''
    return [duration, count].filter(Boolean).join(' · ') || '按计划完成今天这一组'
  }
  return '开始一段运动后，这里会展示今日数据和记录入口'
})
const sessionPrimaryText = computed(() => todayPlan.value?.hasPlan && todayPlan.value.planType !== 'rest' ? '执行计划' : '开始运动')
const sessionSecondaryText = computed(() => todayPlan.value?.hasPlan ? '查看计划' : '去定制')
const workoutTypeMap = {
  1: { name: '跑步', color: 'var(--accent-green)', bg: 'var(--soft-green)' },
  2: { name: '健走', color: 'var(--accent-blue)', bg: 'var(--soft-blue)' },
  3: { name: '骑行', color: 'var(--accent-orange)', bg: 'var(--soft-orange)' }
}
function getWorkoutTypeInfo(type) {
  return workoutTypeMap[type] || { name: '运动', color: 'var(--text-tertiary)', bg: 'var(--soft-neutral)' }
}
const activityColorMap = {
  run: { color: 'var(--accent-green)' },
  strength: { color: 'var(--accent-blue)' },
  yoga: { color: 'var(--accent-blue)' },
  rest: { color: '#94a3b8' },
  custom: { color: '#f97316' }
}
...
function buildWorkoutTags(item) {
  const tags = []
  if (item.duration) tags.push({ text: formatWorkoutDuration(item.duration), color: 'var(--accent-blue)', bg: 'var(--soft-blue)' })
  if (item.distance) tags.push({ text: formatWorkoutDistance(item.distance), color: 'var(--accent-green)', bg: 'var(--soft-green)' })
  if (item.calories) tags.push({ text: `${item.calories}千卡`, color: 'var(--accent-orange)', bg: 'var(--soft-orange)' })
  if (tags.length === 0) tags.push({ text: '运动记录', color: 'var(--text-tertiary)', bg: 'var(--soft-neutral)' })
  return tags
}
// ---- 今日目标 ----
const todayPlan = ref(null)
const typeLabelMap = {
  1: { label: '有氧', color: 'var(--accent-green)', bg: 'var(--soft-green)' },
  2: { label: '力量', color: 'var(--accent-blue)', bg: 'var(--soft-blue)' },
  3: { label: '拉伸', color: 'var(--accent-blue)', bg: 'var(--soft-blue)' },
  4: { label: 'HIIT', color: 'var(--accent-orange)', bg: 'var(--soft-orange)' },
  5: { label: '综合', color: 'var(--text-tertiary)', bg: 'var(--soft-neutral)' },
  6: { label: '休息', color: 'var(--text-tertiary)', bg: 'var(--soft-neutral)' }
}
  ...
  try {
    ...
    if (weeklyRaw) {
      ...
      if (dayPlan) {
        ...
        todayPlan.value = {
          ...
          type: isRest ? '休息' : '运动',
          typeColor: isRest ? 'var(--text-tertiary)' : 'var(--accent-green)',
          typeBg: isRest ? 'var(--soft-neutral)' : 'var(--soft-green)',
          duration: totalDur,
          distanceKm: 0,
          description: '',
          activities: acts,
    ...
    if (plans.length > 0) {
      ...
      if (dayCourses.length > 0) {
        const typeIconMap = { 1: 'run', 2: 'strength', 3: 'stretch', 4: 'hiit', 5: 'hiit', 6: 'rest' }
...
function buildTodayPlanExecutionContext() {
  if (!todayPlan.value?.hasPlan) return null
  const today = new Date()
  const date = formatDate(today, 'YYYY-MM-DD')
  const activities = (todayPlan.value.activities || []).map((act, index) => ({
    id: act.details?.id || `${date}-${index}`,
    index,
    exerciseName: act.name || `训练${index + 1}`,
    type: act.type || 'custom',
    typeDesc: Array.isArray(act.tags) ? act.tags.join(' · ') : (act.type || '运动'),
    durationPlanned: Math.max(0, Number(act.duration || act.details?.duration || 0)) * 60,
    setsPlanned: act.details?.exercises?.length || 1,
    repsPlanned: act.details?.reps || 0,
    restTime: act.details?.restTime || 60,
    targetMuscles: Array.isArray(act.tags) ? act.tags.join('、') : '',
    summary: act.summary || '',
    tags: (act.tags || []).map(tag => ({ text: tag, bg: 'var(--bg-secondary)', color: 'var(--text-secondary)' })),
    source: act,
    status: 0,
    setsCompleted: 0,
    caloriesBurned: 0
  ...
  return {
    ...
    localPlan: true,
    date,
    title: todayPlan.value.name || '今日训练',
    subtitle: `${todayPlan.value.duration || 0}分钟 · ${activities.length}项训练`,
    duration: todayPlan.value.duration || 0,
    activities,
    status: 0,
    startedAt: '',
    completedAt: ''
...
function saveTodayPlanExecutionContext(context) {
  uni.setStorageSync('todayPlanExecution', JSON.stringify(context))
}
function openTodayPlanExecution(index = 0) {
  const context = buildTodayPlanExecutionContext()
  if (!context || !context.activities.length) {
    goRunning()
    return
  }
  saveTodayPlanExecutionContext(context)
  uni.navigateTo({ url: `/pages/task/task-detail?source=todayPlan&index=${index}` })
}
function goToTodaySession() {
  if (todayPlan.value?.hasPlan && todayPlan.value.planType !== 'rest') {
    openTodayPlanExecution(0)
    return
  }
  goToWorkoutList()
}
...
function goToSessionPlanAction() {
  if (todayPlan.value?.hasPlan) {
    openTodayPlanExecution(0)
    return
  }
...
let aiJsonBuffer = ''
const aiQuickTags = PLAN_QUICK_TAGS
...
const handleWsMessage = (data) => {
  switch (data.type) {
    case 'start':
      ...
      aiMessages.value.push({ id: Date.now() + '_' + Math.random().toString(36).slice(2, 7), role: 'assistant', content: '', rawContent: '', displayContent: '', isPlan: false })
      scrollToBottom()
      break
    case 'delta':
    ...
    case 'done':
      ...
      if (aiCurrentAssistantIndex.value >= 0) {
        const msg = aiMessages.value[aiCurrentAssistantIndex.value]
        aiInJsonBlock = false
        const rawContent = msg.rawContent || (msg.content + aiJsonBuffer)
        aiJsonBuffer = ''
...
const sendAiMessage = () => {
  ...
  aiInputText.value = ''
  scrollToBottom()
  const systemPrompt = buildAiSystemPrompt(aiStatsData.value, text)
  const chatMessages = [
...
const usePlan = (planData) => {
  ...
  uni.showActionSheet({
    ...
    success: ({ tapIndex }) => {
      try {
        if (tapIndex === 0) {
          saveAiPlanToWeekly(planData)
          loadTodayPlan()
          uni.showToast({ title: '已保存到本周计划', icon: 'success' })
          return
        }
        if (tapIndex === 1) {
          saveAiPlanToMyTrainingPlans(planData)
...
.home {
  min-height: 100vh;
  background:
    radial-gradient(circle at 18% 0%, var(--page-radial-a), transparent 38%),
    radial-gradient(circle at 86% 8%, var(--page-radial-b), transparent 32%),
    var(--bg-primary);
  padding: 0 28rpx;
...
.sc-action.primary {
  ...
  box-shadow: 0 8rpx 20rpx var(--accent-glow);
}
// 今日训练记录卡片列表
.workout-cards {
  display: flex;
...
.ai-model-selector {
  ...
  border: 1rpx solid rgba(139, 92, 246, 0.16);
  &:active {
    background: var(--bg-secondary);
  }
}
