...
async function initAdmin() {
  ...
  try {
    ...
    if (result.insertId) {
      console.log(`[OK] 管理员账号创建成功，ID: ${result.insertId}`);
    } else {
      console.log(`[OK] 管理员账号已更新: ${ADMIN_CONFIG.username}`);
    }
    // 验证账号
    ...
    console.log(`状态: ${rows[0].status === 1 ? '[OK] 启用' : '[ERROR] 禁用'}`);
